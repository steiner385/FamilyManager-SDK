...
import type { PerformanceMetrics } from '../../../core/events/__tests__/performance/types';
...
// Define a type that matches what supertest.agent returns
type TestAgent = ReturnType<typeof supertest.agent>;

// Helper to convert request body to appropriate format
function getRequestBody(req: TestRequest): BodyInit | undefined {
  if (!req.body) return undefined;
  if (typeof req.body === 'string') return req.body;
  if (Buffer.isBuffer(req.body)) return new Blob([req.body]);
...
export async function setupTestContext(): Promise<TestContext> {
  const agent = supertest.agent(async (req: TestRequest) => {
