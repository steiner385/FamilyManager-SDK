...
describe('PluginManager', () => {
    ...
    describe('Initialization', () => {
        ...
        it('should prevent plugin operations before initialization', async () => {
            ...
            await expect(manager.registerPlugin(plugin)).rejects.toThrow('PluginManager must be initialized before registering plugins');
        });
    });
    describe('Plugin Registration', () => {
        ...
        it('should prevent duplicate plugin registration', async () => {
            ...
            await manager.registerPlugin(plugin);
            await expect(manager.registerPlugin(plugin)).rejects.toThrow('Plugin test-plugin is already registered');
        });
        it('should handle plugin dependencies', async () => {
            ...
            await expect(manager.registerPlugin(plugin)).rejects.toThrow('Missing required dependency: dependency');
        ...
        it('should handle plugin initialization', async () => {
            const initialize = jest.fn().mockImplementation(() => Promise.resolve());
            const plugin = {
                id: 'test-plugin',
                name: 'Test Plugin',
                version: '1.0.0',
    ...
    describe('Plugin Unregistration', () => {
        ...
        it('should prevent uninstalling non-existent plugin', async () => {
            await expect(manager.uninstallPlugin('non-existent')).rejects.toThrow('Plugin non-existent is not registered');
        });
        it('should prevent uninstalling plugin with dependents', async () => {
            ...
            await expect(manager.uninstallPlugin('dependency')).rejects.toThrow('Cannot unregister plugin dependency because it is required by dependent');
        });
