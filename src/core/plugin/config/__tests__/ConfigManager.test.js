...
describe('ConfigManager', () => {
    ...
    it('should apply middleware chain correctly', async () => {
        ...
        const middleware1 = async (config, next) => {
            config.value++;
            await next(config);
        };
        const middleware2 = async (config, next) => {
            config.value *= 2;
            await next(config);
        };
    ...
    it('should emit configuration events', async () => {
        ...
        await configManager.setConfig('test-plugin', config);
        expect(mockEventBus.publish).toHaveBeenCalledWith('config', expect.objectContaining({
    ...
    it('should handle validation failures', async () => {
        ...
        const failingMiddleware = async (config, next) => {
