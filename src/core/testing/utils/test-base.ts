...
export async function connectWithRetry(retries = 3, delay = 1000): Promise<void> {
...
export async function setupBaseTest(): Promise<BaseTestContext> {
  try {
        ...
        try {
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            server.getConnections((err, count) => {
              ...
              else if (count > 0) console.log(`Closing ${count} remaining connections`);
            });
...
export async function cleanupBaseTest(): Promise<void> {
  try {
