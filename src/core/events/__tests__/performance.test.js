...
describe('Event System Performance Tests', () => {
    ...
    it('should handle high volume of events efficiently', async () => {
        ...
        const events = Array.from({ length: eventCount }, (_, i) => createTestEvent('TEST_EVENT', { index: i }));
        const startTime = performance.now();
        await Promise.all(events.map(event => mockEventBus.publish(TEST_CHANNEL, event)));
    ...
    it('should maintain consistent performance with multiple subscribers', async () => {
        ...
        const subscribers = Array.from({ length: subscriberCount }, () => jest.fn().mockResolvedValue(undefined));
    ...
    it('should handle concurrent event publishing efficiently', async () => {
        ...
        const publishers = Array.from({ length: concurrentPublishers }, (_, publisherId) => {
            return Array.from({ length: eventsPerPublisher }, (_, eventId) => createTestEvent('TEST_EVENT', { publisherId, eventId }));
        });
        const startTime = performance.now();
        await Promise.all(publishers.flatMap(events => events.map(event => mockEventBus.publish(TEST_CHANNEL, event))));
        const endTime = performance.now();
