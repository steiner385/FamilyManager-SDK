...
describe('EventBus', () => {
    ...
    describe('Channel Registration', () => {
        ...
        it('should prevent duplicate channel registration', () => {
            eventBus.registerChannel('test-channel');
            expect(() => eventBus.registerChannel('test-channel')).toThrow('Channel test-channel is already registered');
        });
        it('should require channel registration before subscription', () => {
            const handler = createMockHandler();
            expect(() => eventBus.subscribe('unregistered-channel', handler)).toThrow('Channel unregistered-channel is not registered');
        });
    });
    describe('Event Subscription', () => {
        ...
        it('should prevent subscription when stopped', async () => {
            await eventBus.stop();
            const handler = createMockHandler();
            expect(() => eventBus.subscribe('test-channel', handler)).toThrow('Cannot subscribe while EventBus is stopped');
        });
    ...
    describe('Event Emission', () => {
        ...
        it('should prevent emission when stopped', async () => {
            ...
            await expect(eventBus.emit(event)).rejects.toThrow('Cannot emit events while EventBus is stopped');
        });
    ...
    describe('Retry Mechanism', () => {
        ...
        it('should retry failed deliveries', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('Retrying event delivery'), expect.any(Object));
        });
    ...
    describe('Error Handling', () => {
        ...
        it('should handle partial delivery failures', async () => {
            ...
            failureHandler.mockImplementation(() => Promise.reject(new Error('Handler failed')));
        ...
        it('should handle invalid channel emissions', async () => {
            ...
            await expect(eventBus.emit(event)).rejects.toThrow('Channel invalid-channel is not registered');
        });
    ...
    describe('Performance', () => {
        ...
        it('should handle rapid event emissions', async () => {
            const handler = createMockHandler();
            eventBus.subscribe('test-channel', handler);
            const events = Array.from({ length: 100 }, (_, i) => createTestEvent('test-channel', { index: i }));
        ...
        it('should handle concurrent subscriptions and emissions', async () => {
            ...
            const events = Array.from({ length: 10 }, (_, i) => createTestEvent('test-channel', { index: i }));
