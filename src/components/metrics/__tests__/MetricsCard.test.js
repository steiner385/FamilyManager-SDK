...
import { render, screen, userEvent, describe, it, expect } from '../../../testing';
...
describe('MetricsCard', () => {
    ...
    it('renders with data', () => {
        render(_jsx(MetricsCard, { ...mockData }));
    ...
    it('handles negative change values', () => {
        render(_jsx(MetricsCard, { ...mockData, trend: "down", change: -8.5 }));
    ...
    it('handles custom formatters', () => {
        ...
        render(_jsx(MetricsCard, { ...mockData, formatters: formatters }));
    ...
    it('handles custom styles', () => {
        ...
        render(_jsx(MetricsCard, { ...mockData, styles: customStyles }));
    ...
    it('handles click events', async () => {
        ...
        render(_jsx(MetricsCard, { ...mockData, onClick: handleClick }));
        const card = screen.getByTestId('metrics-card');
    ...
    it('handles loading state', () => {
        render(_jsx(MetricsCard, { ...mockData, loading: true }));
        expect(screen.getByTestId('metrics-card-skeleton')).toBeInTheDocument();
        expect(screen.queryByText('Total Revenue')).not.toBeInTheDocument();
    });
    it('handles error state', () => {
        render(_jsx(MetricsCard, { ...mockData, error: "Failed to load metrics" }));
        expect(screen.getByText('Failed to load metrics')).toBeInTheDocument();
        expect(screen.queryByText('Total Revenue')).not.toBeInTheDocument();
    });
    it('handles accessibility requirements', () => {
        render(_jsx(MetricsCard, { ...mockData, ariaLabel: "Revenue metrics card" }));
        const card = screen.getByTestId('metrics-card');
    ...
    it('handles tooltip display', async () => {
        ...
        const user = userEvent.setup();
        render(_jsx(MetricsCard, { ...mockData, tooltip: tooltipText }));
    ...
    it('handles neutral trend', () => {
        render(_jsx(MetricsCard, { ...mockData, trend: "neutral", change: 0 }));
        const changeText = screen.getByText('0%');
        expect(changeText).toBeInTheDocument();
        expect(changeText).toHaveClass('text-gray-500');
    });
});
//# sourceMappingURL=MetricsCard.test.js.map