...
describe('LoadingSpinner', () => {
    ...
    it('applies different sizes correctly', () => {
        const { rerender } = render(_jsx(LoadingSpinner, { size: "small" }));
        expect(screen.getByRole('status')).toHaveClass('h-4 w-4');
        rerender(_jsx(LoadingSpinner, { size: "medium" }));
        expect(screen.getByRole('status')).toHaveClass('h-8 w-8');
        rerender(_jsx(LoadingSpinner, { size: "large" }));
        expect(screen.getByRole('status')).toHaveClass('h-12 w-12');
    });
    it('applies custom className', () => {
        render(_jsx(LoadingSpinner, { className: "text-blue-500" }));
    ...
    it('uses custom label when provided', () => {
        render(_jsx(LoadingSpinner, { label: "Processing" }));
        const spinner = screen.getByRole('status');
