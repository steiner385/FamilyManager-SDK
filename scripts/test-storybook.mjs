...
// Get the story file from command line arguments
const storyFile = process.argv[2];

if (!storyFile) {
  console.error('Please provide a story file name');
  process.exit(1);
}

// Start Storybook in CI mode
console.log('Starting Storybook in CI mode...');
const storybookProcess = execSync('npm run storybook -- --ci --port 6010', {
  stdio: 'pipe'
});

// Wait for Storybook to be ready
try {
  execSync('npx wait-on tcp:6010', {
    stdio: 'inherit',
    timeout: 60000 // 60 second timeout
  });
} catch (error) {
  console.error('Timeout waiting for Storybook to start');
  process.exit(1);
}

// Run tests for the specific story
try {
  console.log(`Running tests for ${storyFile}`);
  execSync(
    `test-storybook --ci --url http://localhost:6010 --test-match "${storyFile}"`,
    { stdio: 'inherit' }
  );
} finally {
  // Cleanup: Kill Storybook process
  try {
    execSync('npm run kill-storybook');
  } catch (error) {
    console.warn('Warning: Could not kill Storybook process');
  }
}
